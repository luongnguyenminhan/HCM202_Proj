...
class ArticleService:
	...
	async def get_article_list(self, pagination: PaginationParams) -> ArticleListResponse:
		...
		with Session(engine) as session:
			...
			statement = select(Post).order_by(desc(Post.created_at)).offset(offset).limit(pagination.limit)
			...
			return ArticleListResponse(articles=articles, pagination=pagination_response)

	async def get_article_detail(self, article_id: int) -> Optional[ArticleDetailResponse]:
	...
	async def get_featured_articles(self) -> FeaturedResponse:
		...
		with Session(engine) as session:
			statement = (
				select(Post).where(Post.is_featured == True).order_by(desc(Post.created_at)).limit(5)  # Top 5 featured
			)
