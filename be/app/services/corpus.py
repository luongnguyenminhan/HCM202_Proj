...
class CorpusService:
    ...
    async def upload_document(
        ...
        try:
            ...
            print_warning(
                f"Failed to parse file bytes smartly, fallback as raw text: {e}"
            )
            try:
        ...
        with Session(engine) as session:
            ...
            for idx, (title_c, content_c) in enumerate(detected, start=1):
                chapter_data = ChapterCreate(
                    ...
                    ordering=idx,
                    summary=(
                        (content_c[:200] + "...") if len(content_c) > 200 else content_c
                    ),
                )
            ...
            session.commit()
            print_success(
                f"Upload flow finished: doc_id={document.id}, chunks={total_chunks}"
            )
    ...
    async def delete_document(self, document_id: int) -> CorpusDeleteResponse:
        ...
        with Session(engine) as session:
            ...
            chunks = session.exec(
                select(Chunk).join(Chapter).where(Chapter.document_id == document_id)
            ).all()
            for ch in chunks:
                session.delete(ch)
            session.commit()
