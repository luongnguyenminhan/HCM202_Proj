...
async def chat_query(query: ChatQuery, rag_service: RAGService = Depends(get_rag_service)):
	"""
	...
	"""
	try:
		print_info(f"/chat/query: question='{query.question[:80]}...' debug={query.include_debug}")
		response = await rag_service.query(question=query.question, include_debug=query.include_debug)
...
async def report_chat_response(
	...
	try:
		...
		response = await report_service.create_report(reference_id=report.reference_id, reason=report.reason, source=report.source)
